...
- [ ] Install and configure Changesets (`pnpm add -D @changesets/cli && pnpm changeset init`)
- [ ] Scope `.changeset/config.json` to `packages/cli` only (exclude `apps/*`)
- [ ] Add Changesets GitHub Action — opens Version PR on merge to `main`, publishes on Version PR merge
- [ ] Update repo-root `README.md` — install + quickstart
- [ ] `npm pack` dry-run — verify tarball contents
- [ ] Create initial changeset (`pnpm changeset`) and merge Version PR to cut `v0.1.0`
- [ ] Tag `v0.1.0` + create GitHub release with changelog (auto-generated by Changesets)
- [ ] Smoke-test on a clean machine: `npm i -g quiver && quiver login && quiver sync`

**Deliverable:** `quiver` is live on npm and usable by anyone. On a fresh machine — `quiver login && quiver sync` — restores all skills. `quiver remove` ensures the lock file stays clean from day one.
